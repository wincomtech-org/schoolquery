...
// print_r($dou->get_show_list());
$smarty->assign('show_one',$dou->get_show_list());
$smarty->assign('show_list', $dou->get_nav());
...
elseif ($rec == 'update') {
    ...
    $firewall->check_token($_POST['token']);
    if($_POST[id]==1){
        $sql = "update " . $dou->table('show') . " SET show_name='$_POST[show_name]'" . $up_file . " , sort = '$_POST[sort]' WHERE id = '$_POST[id]'";
    $dou->query($sql);
    $dou->create_admin_log($_LANG['show_edit'] . ': ' . $_POST[show_name]);
    $dou->dou_msg($_LANG['show_edit_succes'], 'show.php');
}else{
    $sql = "update " . $dou->table('nav') . " SET nav_name='$_POST[show_name]'" . $up_file . " , sort = '$_POST[sort]' WHERE id = '$_POST[id]'";
    $dou->query($sql);
    $dou->create_admin_log($_LANG['show_edit'] . ': ' . $_POST[show_name]);
    $dou->dou_msg($_LANG['show_edit_succes'], 'show.php');
}
