...
elseif ($rec == 'add') {
    ...
    // CSRF防御令牌生成
    $smarty->assign('token', $firewall->set_token('gallery_category_add'));
...
elseif ($rec == 'insert') {
    ...
    // CSRF防御令牌验证
    $firewall->check_token($_POST['token'], 'gallery_category_add');
    
    $sql = "INSERT INTO " . $dou->table('gallery_category') . " (cat_id, unique_id, parent_id, cat_name, keywords, description, sort)" . " VALUES (NULL, '$_POST[unique_id]', '$_POST[parent_id]', '$_POST[cat_name]', '$_POST[keywords]', '$_POST[description]', '$_POST[sort]')";
    $dou->query($sql);
...
elseif ($rec == 'edit') {
    ...
    // CSRF防御令牌生成
    $smarty->assign('token', $firewall->set_token('gallery_category_edit'));
...
elseif ($rec == 'update') {
    ...
    // CSRF防御令牌验证
    $firewall->check_token($_POST['token'], 'gallery_category_edit');
    
    $sql = "update " . $dou->table('gallery_category') . " SET cat_name = '$_POST[cat_name]', unique_id = '$_POST[unique_id]', parent_id = '$_POST[parent_id]', keywords = '$_POST[keywords]' ,description = '$_POST[description]', sort = '$_POST[sort]' WHERE cat_id = '$_POST[cat_id]'";
    $dou->query($sql);
