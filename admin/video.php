...
elseif ($rec == 'add') {
    ...
    // CSRF防御令牌生成
    $smarty->assign('token', $firewall->set_token('video_add'));
...
elseif ($rec == 'insert') {
    if (empty($_POST['title']))
        $dou->dou_msg($_LANG['video_title'] . $_LANG['is_empty']);
    ...
    // CSRF防御令牌验证
    $firewall->check_token($_POST['token'], 'video_add');
    
    $sql = "INSERT INTO " . $dou->table('video') . " (id, cat_id, title, defined, file, content, image, keywords, add_time, description)" . " VALUES (NULL, '$_POST[cat_id]', '$_POST[title]', '$_POST[defined]', '$_POST[file]', '$_POST[content]', '$file', '$_POST[keywords]', '$add_time', '$_POST[description]')";
    $dou->query($sql);
...
elseif ($rec == 'edit') {
    ...
    // CSRF防御令牌生成
    $smarty->assign('token', $firewall->set_token('video_edit'));
...
elseif ($rec == 'update') {
    ...
    // CSRF防御令牌验证
    $firewall->check_token($_POST['token'], 'video_edit');
    
    $sql = "UPDATE " . $dou->table('video') . " SET cat_id = '$_POST[cat_id]', title = '$_POST[title]', defined = '$_POST[defined]' ,file = '$_POST[file]', content = '$_POST[content]'" . $up_file . ", keywords = '$_POST[keywords]', description = '$_POST[description]' WHERE id = '$_POST[id]'";
    $dou->query($sql);
