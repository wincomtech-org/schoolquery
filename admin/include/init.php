...
if(isset($_USER['user_id'])){
	if($_USER['user_id']!=1){
	$sql="SELECT * FROM ".$dou->table('admin')."where user_id='$_USER[user_id]'";
 $query = $dou->query($sql);
 $row = $dou->fetch_array($query);
 $sql="SELECT mid FROM ".$dou->table('rbac_rwm')."where rid='$row[role_id]'";
  $query = $dou->query($sql);
  ...
  $mou=substr($mou, 0, -1);
  $sql="SELECT * FROM".$dou->table('rbac_module')."where mid in ($mou)";
  $query = $dou->query($sql);
  while( $row = $dou->fetch_assoc($query)){
  	$moudle[]=$row;
  }
}else{
	 $sql="SELECT * FROM".$dou->table('rbac_module');
  $query = $dou->query($sql);
  while( $row = $dou->fetch_assoc($query)){
  	$moudle[]=$row;
  }
