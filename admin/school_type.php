 ...
 */
if ($rec == 'default') {
    $smarty->assign('ur_here', $_LANG['school_type']);
    $smarty->assign('action_link', array (
            'text' => $_LANG['school_type_add'],
            'href' => 'school_type.php?rec=add' 
    ));
 ...
 */
elseif ($rec == 'add') {
    $smarty->assign('ur_here', $_LANG['school_type_add']);
    $smarty->assign('action_link', array (
...
elseif ($rec == 'insert') {
    if (empty($_POST['cat_name']))
        $dou->dou_msg($_LANG['school_type_name'] . $_LANG['is_empty']);
    ...
    $time=time();
    $sql = "INSERT INTO " . $dou->table('school_type') . " (id, stype_name, content, addtime, sort)" . " VALUES (NULL, '$_POST[cat_name]', '$_POST[content]', '$time','$_POST[sort]')";
    $dou->query($sql);
    
    $dou->create_admin_log($_LANG['school_type_add'] . ': ' . $_POST[cat_name]);
    $dou->dou_msg($_LANG['school_type_add_succes'], 'school_type.php');
} 
